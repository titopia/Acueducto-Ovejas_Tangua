...
# =========================
# 🔹 Función para obtener datos ThingSpeak
# =========================
def get_data():
    ...
    df["field1"] = pd.to_numeric(df["field1"], errors="coerce")  # altura (m)
    df["field2"] = pd.to_numeric(df["field2"], errors="coerce")  # caudal (L/min)
    df["field3"] = pd.to_numeric(df["field3"], errors="coerce")  # volumen (m³)
    return df
...
with tab4:
    ...
    # Altura del agua proporcional al tanque (altura máxima = 2 unidades)
    ...
    fig.add_trace(go.Mesh3d(
        x=[1,1,-1,-1,1,1,-1,-1],
        y=[1,-1,-1,1,1,-1,-1,1],
        z=[0,0,0,0,2,2,2,2],  # altura total del tanque
        color="lightgrey",
        opacity=0.15,
        name="Tanque",
        alphahull=0
    ...
    fig.update_layout(
        scene=dict(
            ...
            zaxis=dict(range=[0,2], title="Altura (m, escala 2=100%)"),
        ),
